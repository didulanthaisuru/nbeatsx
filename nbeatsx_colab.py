...
# Mount Google Drive (uncomment if your data is in Google Drive)
...
model = NBEATSx(
    h=horizon,                   
    input_size=180,             # Longer lookback window for better pattern recognition
    ...
    loss=MAE(),                 # MAE loss for robustness
    valid_loss=MAE(),           # MAE validation loss
    stack_types=['identity', 'trend', 'seasonality'],  # Added seasonality stack
    n_blocks=[3, 3, 3],         # More blocks for each stack
    mlp_units=[[512, 512], [512, 512], [512, 512]],  # Larger network
    dropout_prob_theta=0.1,     # Moderate dropout
    early_stop_patience_steps=20,  # Increased patience
    val_check_steps=50,         # Regular validation checks
...
files.download('nbeatsx_optimized_model') 