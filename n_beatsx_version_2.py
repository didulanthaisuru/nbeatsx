# -*- coding: utf-8 -*-
"""NBEATSx model for balance forecasting"""
...
# Create and train the NBEATSx model with improved learning parameters
model = NBEATSx(
    h=horizon,                   
    input_size=60,              # Reduced for smaller dataset
    futr_exog_list=exogenous_vars,
    hist_exog_list=exogenous_vars,
    random_seed=42,
    scaler_type='standard',     
    learning_rate=0.0005,       # Reduced learning rate
    max_steps=500,              # Increased max steps
    batch_size=16,              # Reduced batch size
    stack_types=['identity', 'trend'],
    n_blocks=[2, 1],            # Reduced complexity
    mlp_units=[[64, 64], [64, 64]],  # Reduced network capacity
    dropout_prob_theta=0.2,     # Increased dropout
    early_stop_patience_steps=20,
    val_check_steps=20,
    optimizer=AdamW,            # Pass the optimizer class
    optimizer_kwargs={
        'weight_decay': 1e-4,   # Increased weight decay
        'betas': (0.9, 0.999)
    },
    gradient_clip_val=1.0,      # Add gradient clipping
    lr_scheduler='OneCycleLR',  # Add learning rate scheduler
    lr_scheduler_kwargs={
        'max_lr': 0.001,
        'pct_start': 0.3,
        'div_factor': 25.0,
